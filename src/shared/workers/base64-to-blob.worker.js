...
/**
 * Web worker wrapper for image conversion function
 * Necessary to prevent blocking loading animation
 */
function worker() {
  /**
  ...
  function base64ToBlob({image64, contentType = '', sliceSize = 512}) {
    if (!image64 || !image64.length) return ''
  ...
  this.onmessage = ({ data }) => {
    postMessage(base64ToBlob(data))
  }
}


export default buildWorker(worker)
